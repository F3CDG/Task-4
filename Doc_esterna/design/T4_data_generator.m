...
q24_12=quantizer([24 12]); % Define quantizer as (precision, decimal part)
% [a,b]=range(q16_1); % Max & Min value in quantizer range
q24_04=quantizer([24 20]); % Define quantizer as (precision, decimal part)


%%q48_08=quantizer([48 8]); %Define qauntiez as precision,decimal part
%% Generate output:
n=100;               % Number of elements for test
fs=20460000;        % Sampling frequency (Hz)
dt=1/fs;            % Time step (s)
...
%% Write data on file EARLY in binary
...
%% Write data on file LATE in binary
...
%% write in a file r^2
myFile='r2.txt';       % Filename
fid = fopen(myFile,'w');        % Open file
for i=1:length(sigLate) % For each data value
    di2 = num2bin(q48_08,d1(i)); % Generate binary string
...
%% WRITE R in <24,20> precision

myFile='output_R_2404.txt';       % Filename
fid = fopen(myFile,'w');        % Open file
for i=1:length(R) % For each data value
    b = num2bin(q24_04,R(i)); % Generate binary string
    fwrite(fid,[b char(13) char(10)]); % Remember to add \r\n to end lines!
end % For each data value
fclose(fid);                    % Close file

%% WRITE R in <24,12> precision

myFile='output_R_2412.txt';       % Filename
fid = fopen(myFile,'w');        % Open file
for i=1:length(R) % For each data value
    b = num2bin(q24_12,R(i)); % Generate binary string
    fwrite(fid,[b char(13) char(10)]); % Remember to add \r\n to end lines!
end % For each data value
fclose(fid);                    % Close file
